...
local in_snippet = function()
	...
	if pos[1] - 1 >= snip_begin_pos[1] and pos[1] - 1 <= snip_end_pos[1] then return true end
end
		...
		format = function(entry, vim_item)
			...
			if entry.source.name == "nvim_lsp" then
				...
				if color and type(color) == "string" and color:match "^#%x%x%x%x%x%x$" then
					...
					if not vim.api.nvim_get_hl(0, { name = hl }).fg then vim.api.nvim_set_hl(0, hl, { fg = color }) end
...
-- Smart ghost text: only show when cursor is at a word boundary
